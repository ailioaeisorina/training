...
with st.sidebar:
    ...
    # site/room filters
    sites = session.sql("SELECT DISTINCT site FROM azure_view ORDER BY site").to_pandas()["SITE"].dropna().tolist()
    sel_sites = st.multiselect("Site", options=sites, default=sites)

    rooms = session.sql("SELECT DISTINCT room FROM azure_view ORDER BY room").to_pandas()["ROOM"].dropna().tolist()
...
# files_in_stage = session.sql(f"LIST @azure.public.azure_stage").to_pandas()
# st.write("Fișiere în stage:", files_in_stage)
# filename = f"plot_df_{uuid.uuid4().hex}.csv"
# stage_location = f"@azure_stage/streamlit/{filename}"
# path_in_stage = f"streamlit/{filename}"

# csv_bytes = io.BytesIO()
# csv_bytes.write(plot_df.to_csv(index=False).encode('utf-8'))
# csv_bytes.seek(0)

# try:
#     session.file.put_stream(
#         input_stream=csv_bytes,
#         stage_location=stage_location,
#         overwrite=True
#     )
# except Exception as e:
#     st.error(f"❌ Eroare la încărcarea fișierului în stage: {e}")
#     st.stop()

# try:
#     presigned_url = session.sql(
#         f"SELECT GET_PRESIGNED_URL('@azure_stage', '{path_in_stage}')"
#     ).to_pandas().iloc[0, 0]
# except Exception as e:
#     st.error(f"❌ Eroare la generarea linkului de download: {e}")
#     st.stop()

# st.success("✅ Fișierul CSV a fost generat cu succes!")
# st.markdown(f"[📥 Descarcă fișierul CSV]({presigned_url})", unsafe_allow_html=True)

# csv = base_df.to_csv(index=False)  # Avoid including the index if unnecessary
# st.download_button(
